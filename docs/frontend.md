...
```tsx
// Controller actions — use the full PHP namespace path
import {
    index,
    store,
    update,
...
// Named routes — short path based on route name
...
These are auto-generated by the `@laravel/vite-plugin-wayfinder` Vite plugin whenever routes change. The `@/actions/` paths mirror the full controller namespace, while `@/routes/` paths use short names derived from the route names.
